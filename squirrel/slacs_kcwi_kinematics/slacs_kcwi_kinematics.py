...
sys.path.append(
    "/home/shawnknabel/Documents/slacs_kinematics/my_python_packages/ppxf_kcwi_util_022423"
)
...
class slacs_kcwi_kinematics:
    ...
    def run_slacs_kcwi_kinematics(self, fit_poisson_noise=False, plot_bin_fits=False):
        ...
        plot_bin_fits- if True, will plot each ppxf fit for the bins, default is False
        (to save time)
        """
        print(
            f"pPXF will now consume your soul and use it to measure the kinematics of {self.obj_name}."
        )
    ...
    def datacube_visualization(self):
        """Function shows the mosaic'ed datacube summed over the wavelength axis.

        This is mostly just to get oriented to where things are in the image.
        """
    ...
    def log_rebin_central_spectrum(self):
        """Function to deredshift and rebin the central foreground galaxy spectrum to
        log space and prepare the restframe wavelengths for proper fitting with the
        stellar template spectra later."""
        ...
        lamRange1 = h1["CRVAL1"] + np.array([0.0, h1["CDELT1"] * (h1["NAXIS1"] - 1)])
    ...
    def log_rebin_background_spectrum(self):
        """Function to deredshift (to the foreground deflector galaxy redshift zlens)
        and rebin the background source galaxy spectrum to log space and prepare the
        restframe wavelengths for proper fitting with the "sky" keyword in ppxf."""
        ...
        if self.background_spectrum_file is not None:
            ...
            background_source, _, _ = ppxf_util.log_rebin(
                self.rest_wave_range, background_source_lin
            )
    ...
    def get_templates(self):
        """Function prepares the stellar template spectra from the sps model identified
        by sps_name."""

        # take wavelength range of templates to be slightly larger than that of the galaxy restframe
        self.wave_range_templates = (
            self.rest_wave_range[0] / 1.2,
            self.rest_wave_range[1] * 1.2,
        )
    ...
    def set_up_mask(self):
        """Function prepares a mask of wavelengths for ppxf that determines the correct
        wavelength range and masks some gas lines."""
        # take the pixels in the range
        # after de-redshift, the initial redshift is zero.
        goodPixels = ppxf_util.determine_goodpixels(
            self.rest_wave_log, self.wave_range_templates, 0
        )
    ...
    def ppxf_central_spectrum(self):
        """Function fits the central_spectrum with the stellar template spectra, a
        polynomial of specified degree, and the background source as the "sky"
        component."""

        # some setup, starting guesses
        vel = c * np.log(1 + 0)  # eq.(8) of Cappellari (2017)
        start = [vel, 250.0]  # (km/s), starting guess for [V, sigma]
        ...
        # fit with ppxf
        pp = ppxf(
            self.templates,  # templates for fitting
        ...
        plt.plot(
            ...
            data_lin - back_lin,
            "m-",
            label="remove background source from data",
            alpha=0.5,
        )  # if there is a background source to remove
        plt.plot(
            lin_axis,
            back_lin + np.full_like(back_lin, 0.9e-5),
            "c-",
        ...
        plt.plot(
            lin_axis,
            noise_lin + np.full_like(back_lin, 0.9e-5),
            "g-",
        ...
        plt.legend(loc="best")
        plt.ylim(
            np.nanmin(noise_lin[plot_ind_min:plot_ind_max]) / 1.1,
            np.nanmax(data_lin[plot_ind_min:plot_ind_max]) * 1.1,
        )
    ...
    def crop_datacube(self):
        ...
        if self.background_source_mask_file is not None:
            ...
            self.background_source_mask = (
                ~getMaskInFitsFromDS9reg(
                    self.background_source_mask_file, datacube.shape[1:], data_hdu[0]
                )
                * 1
            )
            ...
            plt.imshow(
                np.nansum(datacube, axis=0) * self.background_source_mask,
                origin="lower",
                norm=norm,
            )
        ...
        plt.imshow(
            np.nansum(self.cropped_datacube, axis=0) * self.background_source_mask,
            origin="lower",
            norm=norm,
        )
    ...
    def create_SN_map(self):
        ...
        noise_from_blank = self.cropped_datacube[
            self.wave_min : self.wave_max, 4 - 3 : 4 + 2, 4 - 3 : 4 + 2
        ]
        ...
        lin_axis = np.linspace(
            self.rest_wave_range[0],
            self.rest_wave_range[1],
        ...
        flux_per_half_AA = np.nanmedian(
            self.cropped_datacube[ind_min_SN:ind_max_SN, :, :], axis=0
        )
        ...
        sigma_poisson = poisson_noise(
            self.exp_time, flux_per_AA, sigma, per_second=True
        )
    ...
    def select_region(self):
        """Function to select the pixels that will be used for Voronoi binning by
        minimum pixel SN."""

        # center the SN map on highest value
        SN_y_center, SN_x_center = np.unravel_index(
            self.SN_per_AA.argmax(), self.SN_per_AA.shape
        )
        ...
        self.voronoi_binning_input = np.vstack(
            (xx_1D, yy_1D, SN_1D, np.ones(SN_1D.shape[0]))
        )
    ...
    def voronoi_binning(self):
        """Function takes the pixels and S/N selectd from the "select_region" function
        and bins using Voronoi binning technique."""
        ...
        (
            ...
            nPixels,
            scale,
        ) = voronoi_2d_binning(x, y, signal, noise, self.bin_target_SN, plot=1, quiet=1)
        ...
        self.voronoi_binning_data = np.zeros(
            (
        ...
        for i in range(self.voronoi_binning_output.shape[0]):
            ...
            self.voronoi_binning_data[num] = (
                self.voronoi_binning_data[num] + self.cropped_datacube[:, wy, wx]
            )
        ...
        plt.scatter(
            self.bin_centers[:, 0], self.bin_centers[:, 1], c="k", marker=".", s=2
        )
        plt.colorbar(p)
        plt.pause(1)
    ...
    def ppxf_bin_spectra(self, fit_poisson_noise=False, plot_bin_fits=False):
        """Function to loop through bin spectra constructed with "voronoi_binning"
        function. Fit each spectrum with ppxf using "global_template" constructed during
        the "ppxf_central_spectrum" function.
        ...
        for i in range(self.nbins):
            ...
            galaxy, log_wavelengths, velscale = ppxf_util.log_rebin(
                self.rest_wave_range, bin_spectrum
            )
            ...
            lam_range_global_temp = np.array(
                [self.global_template_wave.min(), self.global_template_wave.max()]
            )

            # keep only the good pixels, after de-redshift, the initial redshift is zero.
            goodPixels = ppxf_util.determine_goodpixels(
                log_wavelengths, lam_range_global_temp, 0
            )
            ...
            pp = ppxf(
                self.global_template,  # global template created in "ppxf_central_spectrum" function
            ...
            self.bin_kinematics = np.vstack(
                (
                    self.bin_kinematics,
                    np.hstack((pp.sol[:2], (pp.error * np.sqrt(pp.chi2))[:2], pp.chi2)),
                )
            )
    ...
    def make_kinematic_maps(self):
        """Function to make 2D maps of kinematic components and errors of all bins
        measured in "ppxf_bin_spectra" function."""
        ...
        self.pixel_details = np.vstack(
            (self.voronoi_binning_output.T, VD_array, dVD_array, V_array, dV_array)
        )
        ...
        self.dVD_2d[:] = np.nan
        for i in range(self.pixel_details.shape[1]):
            self.dVD_2d[int(self.pixel_details[1][i])][
                int(self.pixel_details[0][i])
            ] = self.pixel_details[4][i]
